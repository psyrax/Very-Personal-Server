...
<div class='links'>
    ...
    <hr />
    <span class='namedwnld'>Category: </span>
    <?php
    $optd="class='namedwnld'";
    $options=array();
    foreach ($cats as $K => $V):
    $options[$V->id]=$V->name;
    endforeach;
    echo form_dropdown('cats', $options,'',$optd);
    ?>
    <hr />
    Some links have been previously downloaded: <br />
    <?php
        foreach ($id_jobs as $K):
            echo $job_info[$K]['owner']['0']->user;
            echo $job_info[$K]['0']->name;
            foreach ($findings as $F):
                if($F->job_id==$K) echo $F->link."<br />";
            endforeach;
        endforeach;
    ?>
    <table>
